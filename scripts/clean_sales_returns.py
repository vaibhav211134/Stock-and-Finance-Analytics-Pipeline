...
# ── CONFIG ────────────────────────────────────────────────────────────────────
RAW_FILE   = "raw_data/inventory_transaction_report_sales_return.csv"
CLEAN_FILE = "clean_data/sales_returns_clean.csv"
...
df.columns = [
    ...
    "return_qty", "base_uom", "net_amount", "remarks"
]
...
df["return_date"]         = df["voucher_date"].dt.date
df["return_year"]         = df["voucher_date"].dt.year
df["return_month"]        = df["voucher_date"].dt.month
df["return_month_name"]   = df["voucher_date"].dt.strftime("%b")
df["return_quarter"]      = df["voucher_date"].dt.quarter
df["return_value"]        = df["net_amount"].round(2)
df["return_value_per_unit"] = (
    df["net_amount"] / df["return_qty"].replace(0, float("nan"))
).round(2)
df["has_customer_name"]   = df["customer_ref"].notna()

df = df.drop(columns=["voucher_date", "net_amount"])
...
FORCE_STRING = {
    ...
    "style", "shade", "size", "base_uom", "return_month_name",
}
FORCE_DATE   = {"return_date"}
FORCE_BOOL   = {"has_customer_name"}
FORCE_INT    = {"return_year", "return_month", "return_quarter"}
